...
function Songs(){
			var playl = JSON.parse(playlistsJSON);
			var song=playl[2].songs;
			for(i = 0;i<song.length;i++){
					console.log(song[i].songAuthor, song[i].songTitle,song[i].songLength, song[i].songListened)
		}
}
function AccountLogIn(x,y){
...
for(i = 0;i<users.length;i++){
  ...
  if (x==email && y==pass){
    console.log("login succesfull with account",i+1, email, pass );
  }
  else{
    console.log("login failed with account", i+1);
  }
}
}

function playlist(id, title, songss){
	this.id;
	this.title;
	Songs.call();

}
playlist.prototype= new Songs();
playlist.prototype.constructor=playlist;

AccountLogIn("kathywhitley@quailcom.com","inson2079"); //login will be succesfull with account 2
Songs();
